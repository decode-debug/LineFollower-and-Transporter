...
GRIPPER_CLOSE_SECONDS = (
    0.8  # alternatywnie: czas w sekundach przy on_for_seconds
)
...
def backup_backward(speed_pct=BACKUP_SPEED, seconds=BACKUP_SECONDS):
    ...
    print(
        ">>> Cofam do tyłu przez {:.2f}s z prędkością {}%.".format(seconds, speed_pct)
    )
    try:
...
def state_rotating(now, L, R, left_name, right_name):
    ...
    if success:
        ...
        print(
            ">>> ROTATING zakończone sukcesem — oba sensory widzą '{}'. Przechodzę do FOLLOW_COLOR.".format(
                target
            )
        )
        return "FOLLOW_COLOR"
...
def state_follow_color(now, L, R, left_name, right_name):
    ...
    if left_name == "black" or right_name == "black":
        ...
        color_seen_time = None
        print(
            ">>> Wykryto CZARNY podczas śledzenia '{}'. Wracam do FOLLOW_BLACK.".format(
                prev
            )
        )
        return "FOLLOW_BLACK"
    ...
    if (
        color_seen_time is not None and (now - color_seen_time) > COLOR_LOST_TIMEOUT
    ):
        prev = current_target
        current_target = "black"
        pid.reset()
...
try:
    ...
    while True:
        ...
        elif current_state == "ACTION" and Rotation > Action:
        ...
        if next_state != current_state:
            print(
                ">>> STATE TRANSITION: {} -> {}".format(current_state, next_state)
            )
