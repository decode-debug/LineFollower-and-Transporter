...
class PIDController:
    ...
    def compute(self, setpoint, measured_value):
        ...
        derivative = (
            1.0 - self.d_alpha
        ) * raw_derivative + self.d_alpha * self._previous_derivative
...
pid = PIDController(kp=1.9, ki=0, kd=0, dt=0.01, integral_limit=4, d_alpha=0.2)
...
GRIPPER_CLOSE_SECONDS = (
    0.8  # alternatywnie: czas w sekundach przy on_for_seconds  # noqa: E501
)
...
def read_both_colors():
    ...
    return normalized_color_name(left_color_sensor), normalized_color_name(
        right_color_sensor
    )  # noqa: E501
...
def apply_pid_speed_control(
    current_left_speed, current_right_speed, error_value, gain=1.0
):  # noqa: E501
...
def close_gripper():
    ...
    if not GRIPPER_AVAILABLE:
        ...
        return
    try:
        gripper.on_for_degrees(
            SpeedPercent(GRIPPER_SPEED), -GRIPPER_CLOSE_DEG, block=True
        )
        grip_state = "closed"
...
def open_gripper():
    ...
    try:
        ...
        gripper.on_for_degrees(
            SpeedPercent(GRIPPER_SPEED), -GRIPPER_OPEN_DEG, block=True
        )
        grip_state = "open"
...
def backup_backward(speed_pct=BACKUP_SPEED, seconds=BACKUP_SECONDS):
    ...
    print(
        ">>> Cofam do tyłu przez {:.2f}s z prędkością {}%.".format(seconds,
                                                                   speed_pct)
    )
    try:
...
def state_follow_black(now, L, R, left_name, right_name):
    ...
    if now - last_check_time >= CHECK_INTERVAL:
        ...
        if seen_color_now is not None and seen_color_now not in seen_colors:
            ...
            if initial_side is not None:
                print(
                    ">>> Nowy kolor {} wykryty przez {}. Przechodzę do ROTATING.".format(  # noqa: E501
    ...
    if not is_rotating:
        robot.on(SpeedPercent(leftcirclespeed), SpeedPercent(ricghtcirclespeed))  # noqa: E501

    # sprawdzanie: jeśli oba czujniki widzą ten sam kolor (dowolny), to możliwa akcja  # noqa: E501
    if (
        (not is_rotating) and (left_name is not None) and (left_name ==
                                                           right_name)
    ):  # noqa: E501
...
def state_rotating(now, L, R, left_name, right_name):
    ...
    print(
        ">>> ROTATING: próbuję wyrównać kolor '{}' od strony {}.".format(
            target, initial_side
        )
    )
    success = rotate_until_both_see(target, initial_side,
    ...
    if success:
        ...
        print(
            ">>> ROTATING zakończone sukcesem — oba sensory widzą '{}'. Przechodzę do FOLLOW_COLOR.".format(  # noqa: E501
                target
            )
        )  
        return "FOLLOW_COLOR"
...
def state_follow_color(now, L, R, left_name, right_name):
    ...
    if left_name == "black" or right_name == "black":
        ...
        color_seen_time = None
        print(
            ">>> Wykryto CZARNY podczas śledzenia '{}'. Wracam do FOLLOW_BLACK.".format(  # noqa: E501
                prev
            )
        )  
        return "FOLLOW_BLACK"
    ...
    if (
        color_seen_time is not None and (now - color_seen_time) > COLOR_LOST_TIMEOUT  # noqa: E501
    ):
        prev = current_target
        current_target = "black"
        pid.reset()
...
def state_action(now, L, R, left_name, right_name):
    ...
    robot.off()
    print(
        ">>> ACTION: oba sensory widzą '{}'. Stan chwytaka = '{}'".format(
...
try:
    ...
    while True:
        ...
        if next_state != current_state:
            print(
                ">>> STATE TRANSITION: {} -> {}".format(current_state, next_state)  # noqa: E501
            )  # noqa: E501
